...
*/
    ...
    d_fm_L := float64(0.25e-9);
    d_fm_R := float64(0.25e-9);
...
/*
    ...
    cmplxSparse.ScaleRangeSparseMatrixIP(N_fm_L, N_fm_L+N_ox-1, 2, complex(t_ox, 0.0), Hamiltonian);   
    cmplxSparse.ScaleRangeSparseMatrixIP(N_fm_L+N_ox, N_fm_L+N_ox+N_fm_R-1, 2, complex(t_fm_R, 0.0), Hamiltonian);    

    // Build the upper diagonal
    cmplxSparse.ScaleRangeSparseMatrixIP(2, N_fm_L+1, -2, complex(t_fm_L, 0.0), Hamiltonian);    
    cmplxSparse.ScaleRangeSparseMatrixIP(N_fm_L+2, N_fm_L+N_ox+1, -2, complex(t_ox, 0.0), Hamiltonian);   
    cmplxSparse.ScaleRangeSparseMatrixIP(N_fm_L+N_ox+1, N_fm_L+N_ox+N_fm_R+1, -2, complex(t_fm_R, 0.0), Hamiltonian);    
*/
}
